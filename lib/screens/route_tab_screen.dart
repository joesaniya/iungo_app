            ...
            children: [
              ...
              if (provider.totalWasteCollected != null)
                MetricCard(
                  ...
                  value: provider.totalWasteCollected!.displayWeight,
                  unit: 'kg', // Added unit as kg
                ),
              const SizedBox(height: 16),

              // Zone Distribution Chart - Only show if data is not empty
              if (provider.totalWasteCollected?.chartData != null &&
                  _hasChartData(provider.totalWasteCollected!.chartData!))
                Container(
              ...
              if (provider.collectionCompletion != null)
                MetricCard(
                  ...
                  value: provider.collectionCompletion!.displayWeight,
                ...
                children: [
                  if (provider.co2Emission != null)
                    Expanded(
                      child: MetricCard(
                        title: 'CO₂ Emission',
                        value: provider.co2Emission!.displayWeight,
                        unit: 'kg',
                      ),
                    ),
                  ...
                  if (provider.routeEfficiency != null)
                    Expanded(
                      child: MetricCard(
                        ...
                        value: provider.routeEfficiency!.displayWeight,
                        unit: '%',
              ...
              // Vehicle Emissions Chart - Only show if data is not empty
              if (provider.co2Emission?.chartData != null &&
                  _hasChartData(provider.co2Emission!.chartData!))
                Container(
                  padding: const EdgeInsets.all(20),
                  decoration: BoxDecoration(
                    color: AppColors.white,
  ...
  List<DonutChartData> _getZoneChartData(Map<String, dynamic> chartData) {
    ...
    if (labels != null && values != null) {
      ...
      chartData.forEach((key, value) {
        if (key != 'labels' && key != 'values' && key != 'total_weight') {
          data.add(
            DonutChartData(
              label: key,
